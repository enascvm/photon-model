...
public class AzureInstanceService extends StatelessService {
    ...
    private DeferredResult<AzureInstanceContext> deleteVHDBlobsForUnmangedDisks(
        ...
        return fetchVHDBlobsToDelete(ctx)
        ...
        CompletableFuture.runAsync(() -> {
            diskStatesToDelete.forEach(disk -> {
                try {
                    ...
                    logSevere("Unable to delete VHD file for disk [%s] because of %s ", disk.name, e.getMessage());
                }
            });

        }, getHost().allocateExecutor(this)).whenComplete((obj, throwable) -> dr.complete(obj));

        return dr;
    }

    private DeferredResult<AzureInstanceContext> fetchVHDBlobsToDelete(AzureInstanceContext ctx) {
