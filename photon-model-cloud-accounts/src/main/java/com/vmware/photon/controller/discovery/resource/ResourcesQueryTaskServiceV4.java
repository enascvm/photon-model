...
import static com.vmware.photon.controller.model.UriPaths.RESOURCE_QUERY_TASK_SERVICE_V4;
 ...
 *
 * Resources include: ComputeState, DiskState, NetworkState, NetworkInterfaceState, SubnetState, SecurityGroupState
 */
public class ResourcesQueryTaskServiceV4 extends TaskService<ResourceQueryTaskState> {
    public static final String FACTORY_LINK = RESOURCE_QUERY_TASK_SERVICE_V4;

    private static final String PROPERTY_NAME_QUERY_RESULT_LIMIT = UriPaths.PROPERTY_PREFIX
            + "ResourcesQueryTaskServiceV4.QUERY_RESULT_LIMIT";
    ...
    public static final String CUSTOM_PROPERTY_ENDPOINT_TYPE = "endpointType";
    public static final String QUERY_WILDCARD_TERM = "*";
    public static final String CLOUDACCOUNT_SERVICE = "cloudAccount.service";
    ...
    static {
        ...
        endpointFilterMap.put(ResourceViewState.FIELD_NAME_CLOUD_TYPE,
                EndpointState.FIELD_NAME_ENDPOINT_TYPE);
        endpointFilterMap.put(CLOUDACCOUNT_SERVICE,
                ResourceState.FIELD_NAME_CUSTOM_PROPERTIES + FilterUtils.DOT
                        + EndpointUtils.ENDPOINT_CUSTOM_PROPERTY_NAME_PREFIX_SERVICE_TAG);

        // Legacy fields - maintain the old mapping as well for backwards compatibility.
        // Existing resource groups use these properties in the stored query.
        endpointFilterMap.put(ResourceViewState.FIELD_NAME_CLOUD_ACCOUNT_NAME_LEGACY,
                ResourceState.FIELD_NAME_NAME);
        endpointFilterMap.put(ResourceViewState.FIELD_NAME_CLOUD_TYPE_LEGACY,
                EndpointState.FIELD_NAME_ENDPOINT_TYPE);
    ...
    static {
        ...
        typeFilterMap.put(ResourceViewState.FIELD_NAME_TYPE, TAG_VALUE);
        SUPPORTED_TYPE_FILTER_MAP = Collections.unmodifiableMap(typeFilterMap);
    }

    /**
     * The values in the map are those property names that will require special handling
     */
    public static final Map<String, Function<Query, Query>> SUPPORTED_SPECIAL_HANDLING_MAP;
    static Function<Query, Query> specialHandlingFunction = providedQuery -> {
        // Custom property keys are indexed as case insensitive. So implicitly convert the
        // service name to lowercase.
        String serviceName = providedQuery.term.matchValue != null ?
                providedQuery.term.matchValue.toLowerCase() : CLOUD_ACCOUNT_SERVICE_TAG_DISCOVERY;

        // Form a key like "customProperties.service_tag:<service_name>"
        String transformedKey = SUPPORTED_ENDPOINT_FIELDS_FILTER_MAP
                .get(providedQuery.term.propertyName)
                + serviceName;
        providedQuery.setTermPropertyName(transformedKey);
        providedQuery.setTermMatchValue(ENDPOINT_CUSTOM_PROPERTY_VALUE_SERVICE_TAG);
        providedQuery.setTermMatchType(MatchType.TERM);
        return providedQuery;
    };

    static {
        Map<String, Function<Query, Query>> specialHandlingMap = new HashMap<String, Function<Query, Query>>();
        specialHandlingMap.put(CLOUDACCOUNT_SERVICE, specialHandlingFunction);
        SUPPORTED_SPECIAL_HANDLING_MAP = Collections.unmodifiableMap(specialHandlingMap);
    }
    ...
    // Form a key for discovery service custom property "customProperties.service_tag:discovery"
    public static final String serviceKey = SUPPORTED_ENDPOINT_FIELDS_FILTER_MAP.get(CLOUDACCOUNT_SERVICE)
            + CLOUD_ACCOUNT_SERVICE_TAG_DISCOVERY;
    ...
    public static FactoryService createFactory() {
        TaskFactoryService fs = new TaskFactoryService(ResourceQueryTaskState.class) {
            ...
            public Service createServiceInstance() throws Throwable {
                return new ResourcesQueryTaskServiceV4();
            }
        };
    ...
    public ResourcesQueryTaskServiceV4() {
    ...
    /**
     * Populates the tenant links (aka project links) from the user context if
     ...
     * The service of origin is also determined here. If the service is not specified in the query,
    ...
    private void processRequest(ResourceQueryTaskState state, SubStage nextStage) {
        ...
        // Additionally check if a service term is provided. If not default to discovery.
        try {
            ...
            // if no query provided, filter by default service
            if (state.filter == null || state.filter.query == null) {
                Operation serviceOperation = createDefaultServiceOperation(state.tenantLinks);
                joinOperations.add(serviceOperation);
            } else {
                // Handle endpoint query term transformation
                            ...
                            for (Operation o : ops.values()) {
                                ...
                                if (resultTask.results.documentCount == 0L
                                    ...
                                    if (typeQueryTaskLinks.contains(resultTask.documentSelfLink)
                                        ...
                                        state.typeTagLinks.add(new HashSet<>(resultTask.results.documentLinks));
                    ...
                    try {
                        ...
                        if (queryResult.nextPageLink != null && !queryResult.nextPageLink.isEmpty()) {
                            ...
                            results.nextPageLink = QueryHelper.startStatelessPageService(this,
                                    ResourcesQueryPageServiceV4.SELF_LINK_PREFIX,
                                    pageService,
                                    state.documentExpirationTimeMicros,
    ...
    private QueryTask buildResourcesQueryTask(ResourceQueryTaskState state) {
        ...
        if (state.endpointLinks != null) {
            // for public cloud resource query for resources which have either endpointLink
            // or endpointLinks set to the search criteria
            querySpec.query.addBooleanClause(Query.Builder.create()
                    ...
                    .addInCollectionItemClause(ComputeState.FIELD_NAME_ENDPOINT_LINKS,
                            state.endpointLinks, Occurance.SHOULD_OCCUR)
                    .build());
        }
        ...
        // Filter out the compute types of ZONE, VM_HOST, and ENDPOINT_HOST
        ...
        Query typeQuery = Query.Builder
                ...
                .addFieldClause(ComputeState.FIELD_NAME_TYPE, VM_HOST.name(),
                        Occurance.SHOULD_OCCUR)
                .addFieldClause(ComputeState.FIELD_NAME_TYPE, ZONE.name(), Occurance.SHOULD_OCCUR)
        ...
        // Hide resources that have no endpointLinks.
        Query endpointLinksNotNullQuery = Query.Builder
                .create(Occurance.MUST_OCCUR)
                .addFieldClause(buildCollectionItemName(ResourceState.FIELD_NAME_ENDPOINT_LINKS),
                        QUERY_WILDCARD_TERM, MatchType.WILDCARD,
                        Occurance.MUST_OCCUR)
                .build();

        querySpec.query.addBooleanClause(endpointLinksNotNullQuery);
        ...
        if (querySpec.options.contains(QueryOption.COUNT)) {
            ...
            builder.addOption(QueryOption.EXPAND_CONTENT)
                   ...
                   .addLinkTerm(ResourceState.FIELD_NAME_ENDPOINT_LINKS)
     ...
     * extract supported filter fields and only send those. In case there is a match found for
     * the advanced query filter we build the appropriate query to filter on the attribute
    ...
    /**
     * Creates query tasks to support the filtering criteria for tags.For every tag clause that is for a different
     * type of tag that is detected fire off a separate query else append that to the same query being generated.
     * For e.g. name=some* and name = *thing and category=random will be translated into
     * two separate queries. One with name=some* and name = *thing while the other one
     * will be a separate query for category=random. This logic is implemented by keeping track of every tag type
     * that is encountered in a hashmap and lookup is performed to determine if the created query clause needs to
     * be appended to an existing query task or a new query task needs to be created.
     * @param state
    ...
    private void createTagsQueryTasks(ResourceQueryTaskState state,
        ...
        FilterUtils.findPropertyNameMatch(state.filter.query,
                ...
                query -> {
                    ...
                    Query constructedQuery = createQuery(tagAttributes[1], tagValues[1], query.term.matchType,
                            query.occurance);
                    ...
                    if (tagAttributeAndQueryBuilderMap.containsKey(tagValues[0])) {
                        ...
                        Query externalTagQuery = createQuery(TagState.FIELD_NAME_EXTERNAL, Boolean.TRUE.toString(),
                        ...
                        Query keyConstructedQuery = createQuery(tagAttributes[0], tagValues[0], query.term.matchType,
                                Occurance.MUST_OCCUR);
                        transformedTagsQueryBuilder.addClause(keyConstructedQuery);
                        // Add the clause for the value constructed above
                        transformedTagsQueryBuilder.addClause(constructedQuery);

                        // get list by specific tenant
                        if (state.tenantLink != null) {
                            transformedTagsQueryBuilder.addCollectionItemClause(ResourceState.FIELD_NAME_TENANT_LINKS,
                                    state.tenantLink);
                        }
    ...
    private void createTypeQueryTask(ResourceQueryTaskState state,
        ...
        FilterUtils.transformQuerySpec(state.filter.query, SUPPORTED_TYPE_FILTER_MAP,
                new ArrayList<>(SUPPORTED_TYPE_FILTER_MAP.keySet()), SUPPORTED_SPECIAL_HANDLING_MAP,
                query -> {
        ...
        // get list by specific tenant
        if (state.tenantLink != null) {
            transformedQueryBuilder.addCollectionItemClause(ResourceState.FIELD_NAME_TENANT_LINKS,
                    state.tenantLink);
        }
        // key clause needs to be appended only once and should be a must occur
    ...
    private void createEndpointQueryTask(ResourceQueryTaskState state,
        ...
        // add term for default service if it's not already defined in the query
        if (!FilterUtils.doesQueryMatchGivenFilters(state.filter.query,
                Collections.singleton(CLOUDACCOUNT_SERVICE))) {
            transformedQueryBuilder.addFieldClause(serviceKey,
                    ENDPOINT_CUSTOM_PROPERTY_VALUE_SERVICE_TAG, Occurance.MUST_OCCUR);
        }

        FilterUtils.findPropertyNameMatch(state.filter.query,
                SUPPORTED_ENDPOINT_FIELDS_FILTER_MAP.keySet(),
                query -> {
                    if (query.term.propertyName.equals(ResourceViewState.FIELD_NAME_CLOUD_TYPE)
        ...
        FilterUtils.transformQuerySpec(state.filter.query, SUPPORTED_ENDPOINT_FIELDS_FILTER_MAP,
                new ArrayList<>(SUPPORTED_ENDPOINT_FIELDS_FILTER_MAP.keySet()),
                SUPPORTED_SPECIAL_HANDLING_MAP, query -> {
                    transformedQueryBuilder.addClause(query);
                });
    ...
    private QuerySpecification buildQuerySpecification(ResourceQueryTaskState state) {
        ...
        if (state.filter != null) {
            ...
            FilterUtils.transformQuerySpec(querySpec.query, SUPPORTED_FILTER_MAP,
                    CASE_INSENSITIVE_FIELDS_LIST, SUPPORTED_SPECIAL_HANDLING_MAP, query -> {
                        transformedQueryBuilder.addClause(query);
                    });
                ...
                query -> {
                    if (query.term.matchValue.equals(ABBREVIATED_NETWORK_DOC_KIND)) {
                        ...
                        queryBuilder.addFieldClause(FIELD_NAME_DOCUMENT_KIND,
                                getFullDocKindValue(ABBREVIATED_NETWORK_DOC_KIND), QueryTask.QueryTerm.MatchType.TERM,
                                Occurance.SHOULD_OCCUR);
                        ...
                        // subnetState clause
                        queryBuilder.addFieldClause(FIELD_NAME_DOCUMENT_KIND,
                                getFullDocKindValue(ABBREVIATED_SUBNET_DOC_KIND), QueryTask.QueryTerm.MatchType.TERM,
                                Occurance.SHOULD_OCCUR);

                        // networkInterfaceState clause
                        queryBuilder.addFieldClause(FIELD_NAME_DOCUMENT_KIND,
                                getFullDocKindValue(ABBREVIATED_NET_INTERF_DOC_KIND), QueryTask.QueryTerm.MatchType.TERM,
                                Occurance.SHOULD_OCCUR);
    ...
    private Operation createDefaultServiceOperation(List<String> tenantLinks) {
        Query query = Query.Builder
                .create(Occurance.MUST_OCCUR)
                .addFieldClause(serviceKey, ENDPOINT_CUSTOM_PROPERTY_VALUE_SERVICE_TAG,
                        Occurance.MUST_OCCUR)
                .addKindFieldClause(EndpointState.class).build();
        QueryTask queryTask = QueryTask.Builder.createDirectTask()
                .setQuery(query).build();
        queryTask.tenantLinks = tenantLinks;
        Operation op = createInventoryQueryTaskOperation(this, queryTask);
        return op;
    }
