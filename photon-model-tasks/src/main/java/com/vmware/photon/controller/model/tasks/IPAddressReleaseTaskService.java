...
/**
 * This class is responsible for marking de-allocated IP addresses as available, after a certain time period. When an IP
 address used by a virtual machine is released, it should not be used by other allocations immediately. This is because
 the IP address might be cached at DNS and some other L4 caches. So, we wait until a specified time period, before
 marking the IP address as available.
 */
public class IPAddressReleaseTaskService extends StatelessService {
    ...
    public static final String IP_ADDRESS_RELEASE_PERIOD_SECONDS_PROPERTY = UriPaths.PROPERTY_PREFIX + "ipam.ipaddress.release.period.seconds";
    public static final long DEFAULT_IP_ADDRESS_RELEASE_PERIOD_SECONDS = TimeUnit.MINUTES.toSeconds(30);

    public static final long IP_ADDRESS_RELEASE_PERIOD_SECONDS = Long.getLong(IP_ADDRESS_RELEASE_PERIOD_SECONDS_PROPERTY,
    ...
    /**
     * Retrieves ip addresses that are in RELEASED state for more than IP_ADDRESS_RELEASE_INTERVAL_MS milliseconds.
     *
     * @return List of IP addresses that are in RELEASED state for more than IP_ADDRESS_RELEASE_PERIOD_MS millii seconds.
     */
    private DeferredResult<List<IPAddressService.IPAddressState>> retrieveReleasedIPAddresses() {
        ...
        long ipAddressReleaseCutoffTime = currentTime - TimeUnit.SECONDS.toMicros(IP_ADDRESS_RELEASE_PERIOD_SECONDS);
        ...
        QueryTask.Query getReleasedIPAddressesQuery = QueryTask.Query.Builder.create()
                ...
                .addFieldClause(FIELD_NAME_IP_ADDRESS_STATUS, IPAddressService.IPAddressState.IPAddressStatus.RELEASED.toString())
                .addRangeClause(ServiceDocument.FIELD_NAME_UPDATE_TIME_MICROS, QueryTask.NumericRange.createLessThanRange(ipAddressReleaseCutoffTime))
                .build();
        ...
        return queryByPages.collectDocuments(Collectors.toList())
                .exceptionally(e -> {
                    logWarning(String.format("Failed to retrieve released IP addresses due to error %s", e.getMessage()));
    ...
    private DeferredResult<List<Operation>> markIPAddressesAsAvailable(List<IPAddressService.IPAddressState> ipAddressStates) {
        ...
        for (IPAddressService.IPAddressState ipAddressState : ipAddressStates) {
            Operation patchOp = Operation.createPatch(this, ipAddressState.documentSelfLink)
                    ...
                    .setCompletion((o, e) -> {
                        if (e != null) {
                            logWarning("Failed to mark IP address resource %s as available due to failure %s",
